...
export function mapGeofence(response: GeofenceResponse): Geofence {
  return {
    ...
    parentCode: response.codigo_padre,
    source: response.fuente,
    version: response.version,
    active: response.vigente,
    geometry: response.geometry_simplified ?? response.geometry ?? null,
  };
}

export function toGeofenceFeatureCollection(
  items: Geofence[],
): FeatureCollection {
  const features: Feature[] = items.flatMap((item) =>
    item.geometry
      ? [
          {
            type: "Feature",
