...
| ------ | -------------------------------------------------------- | ------------ | --------- | ------------------------------------------------------ | -------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------ | ------------------------------------------------------- | ---------------- |
| 1.1.1  | Shell público reutilizable (header + nav + footer legal) | PARTIAL      | HIGH      | `App.tsx` L16–54; sin footer                           | Header `AppBar` + nav Eventos/Mapa + CTA login; **sin footer**, login fuera del shell                    | `PublicAppShell` con marca configurable, nav, **footer** (privacidad, contacto), skip link | Footer y config institucional faltantes; shell no envuelve login         | Extraer layout compartido; tokens `--md-sys-*` en barra | TRA-105          |
...
| 1.1.10 | Login administrativo (pre-auth)                          | INCONSISTENT | HIGH      | `AdminEventsPage.tsx` L72–127; `App.css` `.login-form` | `Field` + `Button` DS; shell `admin-page`; foco `--cyber-cyan`                                           | Misma familia que público: shell institucional, formulario M3, enlace privacidad           | Sin shell público; sin link legal; mezcla cyber en labels                | Login dentro de layout institucional                    | TRA-109          |
| 1.1.11 | Mensajes de error auth                                   | OK           | HIGH      | `src/lib/auth-messages.ts`; `LoginPage.tsx`; `http.ts` | Catálogo seguro + `Alert` DS; 401 autenticado → sesión expirada; pantalla rol (`UnauthorizedRoleScreen`) | Mensajes genéricos en UI; detalle sólo en logs                                             | —                                                                        | Ver `docs/design-system/tra-111-auth-session.md`        | TRA-111          |
...
2. ¿Login en `/admin` debe mostrar también nav pública o layout dedicado sin mapa/eventos?
3. ¿Institución responsable del aviso (razón social, domicilio, ARCO) para TRA-108?
4. ¿Publicar geocercas completas en mapa anónimo es política deseada?
